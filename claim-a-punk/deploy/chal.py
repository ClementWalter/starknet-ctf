...
async def deploy(client: AccountClient, player_address: int) -> int:
    ...
    declare_transaction = await client.sign_declare_transaction(
        compiled_contract=Path("compiled/erc721enumerable_mintable.cairo").read_text(),
        max_fee=int(1e16),
    )
    ...
    contract_deployment = await Contract.deploy(
        ...
        constructor_args=[nft_enumerable_mintable_class_hash, client.address],
    )
    ...
    response = await client.execute(
        calls=[
            contract_deployment.deployed_contract.functions["whitelist"].prepare(
                user_1.address
            ),
            contract_deployment.deployed_contract.functions["whitelist"].prepare(
                user_2.address
...
async def checker(
    client: AccountClient, contract: Contract, player_address: int
) -> bool:
    punks_nft_contract_address = (
        await contract.functions["getPunksNftAddress"].call()
    ).address
    punks_nft = await Contract.from_address(punks_nft_contract_address, client)
    player_nft_balance = (
        await punks_nft.functions["balanceOf"].call(player_address)
    ).balance
